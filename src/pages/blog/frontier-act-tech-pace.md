...
I'm not a lawyer, but this reads to me as "any model that breaks out of its testing sandbox has evaded its developer's control".  Under this bill that's a federal report due in 72 hours, period.

It's essentially impossible to make a networked environment truly secure; the perimeter always leaks. So if every escape is a mandatory report, a lab can't just patch the sandbox and move on... instead they might have to make models that don't try to escape.  That's probably great, but remember there are two ways to make escapes stop:

- align the model so it stops trying, or
- teach it to try escapes only when nobody's watching.

From outside, both look like fewer reports, but the second is more concerning. The drafters seem aware of it, since the deception trigger is scoped to behavior outside of designed evals (which is the stealth case). Mandatory reporting isn't an alignment guarantee, but it's legitimate pressure against pure "speed at all costs", and I'll take it.
...
I think aviation has done an excellent job over the years of working out the game theory so that everyone remains focused on the overall goal: safety (rather than distractions, like "blame").  Near-miss incidents which are reported voluntarily and promptly almost always qualify for immunity from discipline, so no one tries to keep things secret out of fear.  And it's paid off, because commercial aviation achieves an accident rate that's about one fatal accident per 100 million flights.  The precise details of the disclosure-and-immunity deal vary from role to role within aviation, and would probably need to vary in AI Governance as well, but there are many great lessons there.
